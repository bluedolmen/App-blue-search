...
define(["dojo/_base/declare",
        ...
        "org/bluedolmen/alfresco/share/search/SearchResultPropertyLink",
   ...
   return declare([Row], {
      ...
      createDescriptionRenderer: function alfresco_search_AlfSearchResult__createDescriptionRenderer() {
         // Render a description if available...
        	    ...
        	    try {
        	    		var seance = item.jsNode.properties["de:seance"] == null ? "Conseil" : item.jsNode.properties["de:seance"];
            	    var aDate = item.jsNode.properties["de:dateseance"].iso8601;
            	    var aSeanceDate = aDate.substr(8,2) + "/" + aDate.substr(5,2) + "/" + aDate.substring(0,4);
	        	    	item.description = seance + " du " + aSeanceDate;
        	    } catch (e) {
        	    		// Pb while computing the seance
      ...
      createDisplayNameRenderer: function alfresco_search_AlfSearchResult__createDisplayNameRenderer() {
         // jshint nonew:false
    	  	 ...
    	  	 var item = this.currentItem;
    	  	 var title = "";
    	  	 var propertyToRender = "title";
 	     try {
 	    	    var numdelib = item.jsNode.properties["de:numdelib"] ;
	    	 	var num = numdelib == null || numdelib == "" ? "" : numdelib + " - ";
	    	 	var objetdelib = item.jsNode.properties["de:objetdelib"];
	    	 	var obj = objetdelib == null || objetdelib == "" ? "" : objetdelib;

	    	 	title = num + obj;
	    	 	if (title == null || title == "") {
		    	    propertyToRender = "displayName";
	    	 	} else {
		    	 	var annexe = item.jsNode.properties["de:annexe"] == "true" ? " (Annexe)" : "";
	    	 		item.title = title + annexe;
	    	 	}
	     } catch (e) {
	    		// Pb while computing the title
	    	    propertyToRender = "displayName";
	     }
	     ...
	     var config = {
            ...
            currentItem: item,
            pubSubScope: this.pubSubScope,
            propertyToRender: propertyToRender, // title|displayName
            renderSize: "large",
            newTabOnMiddleOrCtrlClick: this.newTabOnMiddleOrCtrlClick,
            defaultNavigationTarget: this.navigationTarget,
            highlightValue: this.highlightValue,
      ...
      createTitleRenderer: function alfresco_search_AlfSearchResult__createTitleRenderer() {
	  	/*alert(this.currentItem.jsNode.properties["de:numdelib"]);*/
